export const transactionsHints = {
  ...
  oracleQueryTx: 'Queries an oracle. The oracle can be queried by any account and requires the oracle fee to be provided. In case the oracle operator does not respond in time, the oracle fee will be returned.',
  ...
  channelCloseSoloTx: 'Closes a state channel unilaterally. This is only necessary if one peer stops responding or cooperating but can also be used by a malicious peer trying to close a channel with a state that hasn\'t been agreed on by all participants. A solo close has to be settled via a ChannelSettleTx after the lock period has passed. The lock period is being agreed upon when opening the channel.',
